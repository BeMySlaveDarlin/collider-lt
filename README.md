...
### 🗑️ **DELETE /events** - Удаление событий
```
DELETE /events?before=2025-01-01T00:00:00Z
```
**Особенности**:
- Сбрасывает кеш (изменение!)
- Тестируется ИЗОЛИРОВАННО от других операций
- Прогрессивные даты: -1 неделя +1 час за каждый тест
...
### 3. Load test: Delete (RPS) - ИЗОЛИРОВАННО
```bash
k6 run --env BASE_URL=http://localhost tests/scenario-delete-isolated.js
```
- **Изолированно**: Тестируется отдельно от других операций
- **Прогрессивные даты**: Начиная с -1 неделя, +1 час за тест
- **Цель**: 3+ RPS удаления

### 4. Load test: Create/Read (RPS)
```bash
...
- **Без DELETE**: Только CREATE/READ операции

## Ключевые изменения в логике

### DELETE операции
- ❌ **Убраны** из смешанных тестов (create-read, score)
- ✔ **Отдельный тест** scenario-delete-isolated.js
- 🗓️ **Прогрессивные даты**: -1 неделя + (номер_теста * 1 час)
...
cat results/delete-isolated-20241215.json | jq '.metrics.deleted_events_count.values'
